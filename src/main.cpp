...
// API Configuration
#define API_URL "your-api-domain.com"  // Replace with your actual API URL
...
bool sendHTTPPost()
{
  ...
  String dataSentResp = readGSMResponse(5000); // Reduced from 10000
  ...
  { // Reduced from 30000 to 15000 ms
  ...
  if (actionResponse.indexOf("+HTTPACTION") != -1)
  {
    ...
    if (firstComma != -1 && secondComma != -1)
    {
      ...
      if (code == 200)
      {
        ...
        // Read HTTP response
        // delay(500);  // Removed delay to speed up processing
        gsmSerial.println(F("AT+HTTPREAD"));
        String httpResponse = readGSMResponse(2000); // Reduced from 5000
        ...
        gsmSerial.println(F("AT+HTTPTERM"));
        waitForResponse("OK", 1000); // Reduced from 2000

        return true; // Success on 200 status
      }
      else if (code == 0)
      {
